...
(* given a string list of all topics, this one gives you list of topic type *)
...
(* given a topic/key word, this one gives you a list of tokenized word of the content of the topic *)
...
(* returns topic_dict data type that has the topic and the counter dictionary via a representation of record *)
...
(* returns a list of topic_dict type from a list of topics *)
...
(* this function actually creates a list of topic_dict from the topic list we have above *)
let all_topic_dict_counter = all_full_topics topics j []

(* a getter function that allows us to use List.map in the following *)
let get_counter topic_dict =
  topic_dict.counter

(* print helper function to visualize `topic_dict list` from `all_topic_dict_counter` *)
let rec print_topic_dict_list = function 
  |  [] -> ()
  |  (e:topic_dict)::l -> print_string e.topic ; print_string " | " ; print_topic_dict_list l

(* given a word, this function checks every topic_dict generated from the above topic list, and if the word is in the topic_dict's counter dictionary, then we add that entire topic_dict type to the accumulator  *)
...
(* the above function only process one word; this one just patterns matches a string list and merge each `topic_dict list` into a whole list with removing duplicates *)
...
(* a wrapper pretty much that uses tokenize a string into a string list and apply the above function to generate a `topic_dict list` and use `Similarity.remove_dups` to remove duplicates*)
