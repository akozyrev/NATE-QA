module Counter = struct
  ...
  let rec mem (word:string) (dict:(string * int) list) : bool = 
    match dict with
    |(s,i)::lst -> if s == word then true else mem word lst
    |[] -> false

  let mem (word:string ) (dict:(string * int) list)=
    let dict_words = List.map (fun a -> fst a) dict 
